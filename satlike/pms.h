...
void Satlike::continue_from_init_solution(vector<int> init_solution,
...
void Satlike::local_search_for_bmo(vector<int>& init_solution,
                                   int* solver_stage) {
  settings();
  ...
  for (step = 1; step < max_flips; ++step) {
    if (hard_unsat_nb == 0 &&
        (soft_unsat_weight < opt_unsat_weight || best_soln_feasible == 0)) {
      if (best_soln_feasible == 0) {
        best_soln_feasible = 1;
    ...
    if (hard_unsat_nb == 0 && (soft_unsat_weight < local_opt_unsat_weight ||
                               local_soln_feasible == 0)) {
      if (soft_unsat_weight < top_clause_weight) {
